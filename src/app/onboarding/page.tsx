      ...
      <div className="space-y-6 py-8">
        ...
        <div className="grid grid-cols-1 gap-4 max-w-sm mx-auto">
          {[
            ...
            <div key={i} className="flex items-center gap-3 p-4 rounded-14 bg-surface-2 border border-white/[0.07]">
    ...
    content: (
      <div className="grid grid-cols-2 gap-4 py-8 max-w-2xl mx-auto">
        {[
          ...
          { id: 'screen', emoji: '📱', title: 'Monitor screen time', desc: 'Track & limit app usage' },
        ].map((intent) => (
          <button
            key={intent.id}
          ...
          >
            ...
            <p className="font-display text-[15px] font-bold text-text-1">{intent.title}</p>
            <p className="text-[12px] text-text-3 mt-1 leading-relaxed">{intent.desc}</p>
      ...
      <div className="max-w-md mx-auto py-12 space-y-8">
        <div className="space-y-6">
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-text-3 uppercase tracking-wider ml-1">Habit Name</label>
            <input 
              type="text" 
          ...
          <div className="space-y-4">
            <label className="text-[12px] font-medium text-text-3 uppercase tracking-wider ml-1">Frequency</label>
            <div className="flex p-1 bg-surface-2 rounded-14 border border-white/[0.07]">
              <button className="flex-1 py-2.5 text-sm font-bold text-text-1 bg-surface-3 rounded-10 shadow-sm">Daily</button>
              <button className="flex-1 py-2.5 text-sm font-medium text-text-3 hover:text-text-2">Weekdays</button>
              <button className="flex-1 py-2.5 text-sm font-medium text-text-3 hover:text-text-2">Custom</button>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {['Morning run', 'Meditate', 'Read 20 pages', 'Drink water', 'Journaling'].map(chip => (
            <button key={chip} className="px-4 py-2 rounded-full border border-white/[0.07] bg-surface-2 text-[13px] text-text-3 hover:text-text-1 hover:border-white/[0.14] transition-all">
              {chip}
            </button>
          ))}
        </div>
      ...
      <div className="flex flex-col items-center text-center gap-8 py-12">
        ...
        <div className="space-y-3 max-w-sm mx-auto">
          {[
            ...
            <div key={i} className="flex items-center gap-3 p-3 bg-surface-2 rounded-10 border border-white/[0.07]">
              <CheckCircle2 className="size-5 text-success" />
              <span className="text-[14px] text-text-2 font-medium">{item}</span>
            </div>
          ))}
        </div>
      </div>
            ...
            {steps.map((_, i) => (
              <div 
                ...
                className={cn(
                  ...
                  i === currentStep ? "w-8 bg-brand" : i < currentStep ? "w-4 bg-success" : "w-4 bg-white/10"
                )} 
              />
            ))}
          </div>
